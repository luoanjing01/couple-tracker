-- ============================================================
--  清除所有数据 + 确保配对码唯一（在 Supabase SQL Editor 全选执行）
...
-- ③ 清除可能已存在的 couple_code 唯一约束，重新添加
...
-- ⑤ 重写 pair_by_code：不改 couple_code，用 partner_id 双向绑定
...
BEGIN
    ...
    -- 双向设置 partner_id（不改 couple_code！）
...
-- 完成
SELECT '✅ 数据已清除，配对码唯一约束已启用' AS result;
